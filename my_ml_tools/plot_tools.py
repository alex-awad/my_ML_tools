...
class PlotTools:
    ...
    def get_limits(predictions, targets):
        ...
        min_value = (
            predictions.min()
            if predictions.min() < targets.min()
            else targets.min()
        )
        ...
        max_value = (
            predictions.max()
            if predictions.max() > targets.max()
            else targets.max()
        )
        ...
        return (min_value - 0.1 * abs(diff), max_value + 0.1 * abs(diff))
    ...
    def parity_plot_train_test(
        ...
        sns.scatterplot(
            x=tr_targets, y=tr_predictions, ax=ax_train, s=marker_size
        )
        sns.scatterplot(
            x=tes_targets,
            y=tes_predictions,
        ...
        if (train_lim is not None) and (test_lim is not None):
            ...
            if (
                tr_predictions.min() < train_lim[0]
                or tr_targets.min() < train_lim[0]
            ):

                print("Train limit or predictions are below plot limits!")

            if (
                tr_predictions.max() > train_lim[1]
                or tr_targets.max() > train_lim[1]
            ):

                print("Train limit or predictions are above plot limits!")

            if (
                tes_predictions.min() < test_lim[0]
                or tes_targets.min() < test_lim[0]
            ):

                print("Test limit or predictions are below plot limits!")

            if (
                tes_predictions.max() > test_lim[1]
                or tes_targets.max() > test_lim[1]
            ):
    ...
    def parity_plot_single(
        ...
        sns.scatterplot(x=targets, y=predictions, ax=ax, s=marker_size)
        ax.plot(x, y, c="r")
